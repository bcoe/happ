...
function init() {
  ...
  app.get('/v1/habits-daily', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    ...
    const data = await Sentry.startSpan(
      ...
      async (span) => {
        ...
        const cachedValue = Math.random() < 0.95 ? [{date: date}] : undefined
        // const cachedValue = Math.random() < 0.0 ? [{date: date}] : undefined
        if (span) {
        ...
        if (span) {
          span.setAttribute('cache.item_size', Math.random() * 256);
        }

        const s = parseInt(1 + Math.random() * 2)
        await db.raw(`select pg_sleep(${s})`)
        return {};
  ...
  app.get('/v1/test-statuses', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    const statuses = [200, 201, 202, 301, 302, 304, 429, 500, 200, 200, 200, 200, 200];
    const status = statuses[parseInt(Math.random() * statuses.length)]
    console.info(`https://httpstat.us/${status}`);
    const resp = await global.fetch(`https://httpstat.us/${status}`, {
      method: 'GET'
    });
    const text = await resp.text();
    res.status(200).send(`${status} ${resp.status}`)
  });

  app.get('/v1/ml-workflow', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    const obj = {
      uuid: crypto.randomUUID(),
      date: Date.now()
    }
    const data = JSON.stringify(obj, null, 2);
    // const resp = await global.fetch(`https://the-internet.herokuapp.com/status_codes/402`, {
    const resp = await global.fetch(`https://the-internet.herokuapp.com/status_codes/200`, {
      method: 'GET',
    });
    const text = await resp.text();
    res.type('json');
    res.status(200).send({workflowStatus: resp.status})
  });

  app.get('/v1/slow', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    const wait = 2000 * Math.random() + 1500;
  ...
  app.get('/v1/internal-api-call', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    const resp = await global.fetch(`http://127.0.0.1:3000/v1/slow`, {
      method: 'GET'
    });
    const text = await resp.text();
    res.status(200).send(`${text} ${resp.status}`)
  });
