...
function init() {
  ...
  app.post('/v1/habits', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    ...
    if (!name) {
      ...
      await db.transaction(async () => {
        const insertHabitResp = (await db('habits').insert({
          name,
          user_id: req.userId,
        }, ['id']))[0];
        ...
        const maxPos = await db('habits_active_position')
          .max({ position: 'position' })
          .where('user_id', req.userId);
        // If no position response exists, assume this is the first active habit:
        if (maxPos.length === 0 || maxPos[0].position === null) {
          await db('habits_active_position').insert({
            habit_id: id,
            user_id: req.userId,
          });
        } else {
          const positionResp = await db('habits_active_position')
            .where('position', maxPos[0].position)
            .where('user_id', req.userId)
            .select(['habit_id', 'position']);
          await db('habits_active_position').insert({
            habit_id: id,
            prev_habit: positionResp[0].habit_id,
            user_id: req.userId,
          });
          await db('habits_active_position')
            .where('user_id', req.userId)
            .where('position', positionResp[0].position)
            .update({
              next_habit: id,
            });
        }
  ...
  app.get('/v1/habits-daily', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    ...
    let habitsDaily = await db('habits_daily')
      .select(['name', 'status', 'habit_id', 'status'])
    ...
    console.info(habitsDaily);
    habitsDaily.forEach((h) => {
      lookup[h.habit_id] = h;
    });
    console.info(lookup);
    positions.forEach((position) => {
      results.push(lookup[position.habit_id]);
    });
