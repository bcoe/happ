...
import { spanToTraceHeader, getDynamicSamplingContextFromSpan, dynamicSamplingContextToSentryBaggageHeader } from '@sentry/core';
...
async function init() {
  ...
  app.get('/v1/metrics', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    ...
    const metrics = await db('daily_habit_metrics')
      ...
      .orderBy('date', 'asc');
    res.type('json');
    res.status(status).send(metrics.slice(-30));
  });

  app.post('/v1/notes', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    const date = (new Date()).toLocaleString('lt', { timeZone: 'Europe/London' }).split(' ')[0];
    const { note } = req.body;
    if (!note) {
      const status = 422;
      res.status(status).send(JSON.stringify({
      ...
      await db('notes')
        .insert({ user_id: req.userId, date, note })
  ...
  app.get('/v1/notes/today', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    const date = (new Date()).toLocaleString('lt', { timeZone: 'America/New_York' }).split(' ')[0];
    const row = await db('notes').where({ user_id: req.userId, date }).first();
    res.type('json');
    res.send({
      note: row ? { note: row.note } : null,
    });

    console.info('emitted notes today');
    // Counter — increment a metric (defaults to +1)
    Sentry.metrics.distribution('www.zombo.com', 100 + Math.random() * 200, {
      unit: 'awesome'
    });
  });

  app.get('/v1/notes', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
