...
const MONGO_DB_NAME = `happ-${process.env.NODE_ENV}`;
...
async function init() {
  // Connect to the MongoDB database used for tracking daily notes.
  ...
  app.post('/v1/notes', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    ...
    if (!note) {
      ...
      const collection = mongoDB.collection('notes');
      const id = `${req.userId}-${date}`;
      const query = { _id: id };
      const update = {
        $set: {
          _id: id, note, date: new Date(date), userId: req.userId,
        },
      };
