  ...
  app.get('/v1/habits-daily', (req, res, next) => helpers.authenticated(req, res, next), async (req, res) => {
    ...
    const data = await Sentry.startSpan(
      ...
      async (span) => {
        ...
        const hit = !!cachedValue && cachedValue.length && cachedValue[0].date.includes(date);
        if (span) {
          span.setAttribute('cache.hit', hit);
        }
        console.info(`cache ${hit ? 'hit' : 'miss'} fetching ${CACHED_HABITS_SESSION_KEY}`);
        if (cachedValue && cachedValue.length && cachedValue[0].date.includes(date)) {
